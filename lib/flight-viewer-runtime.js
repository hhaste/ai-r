...
function focusCameraOnFlight(flight) {
  ...
  camera.pitch = clamp(flight.on_ground ? 0.34 : 0.42, CAMERA_MIN_PITCH, CAMERA_MAX_PITCH)
...
function getKeyboardPanDirection() {
  ...
  if (state.pressedPanKeys.has("ArrowLeft") || state.pressedPanKeys.has("KeyA")) {
    horizontal -= 1
  }
  if (state.pressedPanKeys.has("ArrowRight") || state.pressedPanKeys.has("KeyD")) {
    horizontal += 1
  }
...
function buildAircraftFaces(flight, basis) {
  const headingRadians = ((flight.display_heading_deg ?? 0) * Math.PI) / 180
  const pitchRadians = flight.on_ground ? 0 : pitchFromTrail(flight.trail)
...
function pitchFromTrail(trail) {
  ...
  const previous = trail[trail.length - 2]
  const current = trail[trail.length - 1]
  const altitudeDeltaMeters = getTrailAltitudeDeltaMeters(trail) ?? 0
  const horizontalDistanceMeters = worldDistanceMeters(previous, current)
  ...
  return clamp(Math.atan2(altitudeDeltaMeters, horizontalDistanceMeters), -0.3, 0.3)
}
