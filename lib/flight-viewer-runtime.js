import { ADVISORIES_UPDATED_EVENT } from "./advisory-events"
import {
  AIRSPACE_SETTINGS_STORAGE_KEY,
  DEFAULT_AIRSPACE_ID,
  cloneAirspaceAirports,
  computeAirspaceRegion,
  getAirspacePresetById,
  getDefaultAirspacePreset
} from "./airspace-presets"
...
const SETTINGS_STORAGE_KEY = AIRSPACE_SETTINGS_STORAGE_KEY
...
const AIRCRAFT_ZOOM_SIZE_BOOST = 0.28
...
const EMERGENCY_SQUAWK_CODES = [7500, 7600, 7700]
...
const DEFAULT_AIRSPACE_PRESET = getDefaultAirspacePreset()
const DEFAULT_REGION = computeAirspaceRegion(DEFAULT_AIRSPACE_PRESET)
const DEFAULT_AIRPORTS = cloneAirspaceAirports(DEFAULT_AIRSPACE_PRESET.airports)
const camera = {
...
function loadSettings() {
  const defaults = {
    selectedAirspaceId: DEFAULT_AIRSPACE_ID,
  ...
  try {
    ...
    return {
      ...merged,
      selectedAirspaceId: getAirspacePresetById(merged.selectedAirspaceId).id,
...
const state = {
  ...
  toastTimer: null,
  dismissedAdvisoryKeys: new Set(),
  simulatedPriorityFlight: null,
...
function resetRuntimeState() {
  ...
  const selectedAirspacePreset = getAirspacePresetById(state.settings.selectedAirspaceId)
  state.region = computeAirspaceRegion(selectedAirspacePreset)
  state.airports = cloneAirspaceAirports(selectedAirspacePreset.airports)
  ...
  state.dismissedAdvisoryKeys = new Set()
  state.simulatedPriorityFlight = null
  state.pressedPanKeys = new Set()
  state.lastFrameTimeMs = null
  state.fetchRequestToken = 0
}
...
export function mountFlightViewer(elements) {
  ...
  airportList = elements.airportList ?? null
  ...
  airspaceSelect = elements.airspaceSelect
...
const FLIGHT_COLOR_THEMES = {
  ...
  priority: {
    base: { r: 255, g: 124, b: 112 },
    outline: "rgba(255, 230, 230, 0.88)",
    line: "#ff6d64",
    trail: "rgba(255, 109, 100, 0.92)"
  },
  search: {
...
function getSectorWeatherSummary() {
  ...
  const altimeter = averageAltimeter == null ? "--" : `${averageAltimeter.toFixed(2)} "Hg`
  const visibility =
...
function emitAdvisoriesUpdated(advisories = []) {
  if (typeof window === "undefined") {
    return
  }

  window.dispatchEvent(
    new CustomEvent(ADVISORIES_UPDATED_EVENT, {
      detail: {
        advisories: advisories.map((advisory) => ({
          key: getAdvisoryDismissKey(advisory),
          severity: advisory.severity || "info",
          label: advisory.label || "AI Assist",
...
function hideToastAlert() {
  ...
  emitAdvisoriesUpdated([])
}

function dismissToastCard(card) {
  if (!(card instanceof HTMLElement) || !toastAlert.contains(card)) {
    return
  }

  const dismissKey = card.dataset.dismissKey
  if (dismissKey) {
    state.dismissedAdvisoryKeys.add(dismissKey)
...
function createToastCard({
  ...
  card.className = `toast-card severity-${severity}`
  if (dismissKey) {
...
function updateSettingsControls() {
  airspaceSelect.value = state.settings.selectedAirspaceId
...
function resetDisplayedFeedForAirspace() {
  const selectedAirspacePreset = getAirspacePresetById(state.settings.selectedAirspaceId)
  ...
  state.lastUpdatedMs = null
  state.lastOpenSkyTime = null
  state.feedNoteText = `Loading ${selectedAirspacePreset.name}...`
  state.sourceModeText = "Waiting"
  state.hoveredFlightIcao = null
  state.highlightedConflictIcaos = new Set()
  state.dismissedAdvisoryKeys = new Set()
...
function getEmergencySquawkCode(flight) {
  const squawk = Number.parseInt(flight.squawk || "", 10)
  return EMERGENCY_SQUAWK_LABELS[squawk] ? squawk : null
}

function hasEmergencySquawk(flight) {
  return getEmergencySquawkCode(flight) != null
}
...
function buildAdvisoryDismissKey(type, ...parts) {
  return `${type}:${parts.map((part) => String(part)).join(":")}`
}

function getAdvisoryDismissKey(advisory) {
  return advisory?.dismissKey || ""
}

function filterDismissedAdvisories(advisories) {
  const activeDismissKeys = new Set(
    advisories
      .map((advisory) => getAdvisoryDismissKey(advisory))
      .filter(Boolean)
  )
  const nextDismissedAdvisoryKeys = new Set()

  state.dismissedAdvisoryKeys.forEach((dismissKey) => {
    if (activeDismissKeys.has(dismissKey)) {
      nextDismissedAdvisoryKeys.add(dismissKey)
    }
  })

  state.dismissedAdvisoryKeys = nextDismissedAdvisoryKeys

  return advisories.filter((advisory) => {
    const dismissKey = getAdvisoryDismissKey(advisory)
    return !dismissKey || !state.dismissedAdvisoryKeys.has(dismissKey)
  })
}
...
function classifyPairConflict(left, right) {
  ...
  return {
    ...
    dismissKey: buildAdvisoryDismissKey(
      "conflict",
      ...[left.icao24, right.icao24].sort()
    )
  }
}
...
function createSyntheticCallsign() {
  const suffix = String(1000 + Math.floor(Math.random() * 9000))
  return `SIM${suffix}`
}

function createSyntheticFlightId(callsign, fallbackSeed) {
  const compactCallsign = callsign.replace(/\s+/g, "").toLowerCase()
  return compactCallsign || fallbackSeed
}
...
function buildControllerAdvisories() {
  ...
  state.flights.forEach((flight) => {
    const squawk = getEmergencySquawkCode(flight)
    const emergencyLabel = squawk ? EMERGENCY_SQUAWK_LABELS[squawk] : null
    ...
    advisories.push(
      createBannerAdvisory(
        ...
        0,
        {
          dismissKey: buildAdvisoryDismissKey("priority", flight.icao24, squawk)
        }
      )
    )
  })
        ...
        createBannerAdvisory(
          ...
          spacingNm,
          {
            dismissKey: buildAdvisoryDismissKey(
              "arrival",
              airport.code,
              leader.flight.icao24,
              follower.flight.icao24
            )
          }
        )
      )
  ...
  airborneFlights.forEach((flight) => {
    ...
    advisories.push(
      createBannerAdvisory(
        ...
        {
          dismissKey: buildAdvisoryDismissKey("reroute", flight.icao24, airportContext.airport.code)
        }
      )
    )
  })

  return filterDismissedAdvisories(
    advisories
    .sort((left, right) => {
      const severityDelta = ADVISORY_SEVERITY_RANK[left.severity] - ADVISORY_SEVERITY_RANK[right.severity]
      return severityDelta || left.score - right.score
...
function renderAdvisories() {
  ...
  const visibleAdvisories = pinnedConflictAdvisory
    ? [
        pinnedConflictAdvisory,
        ...state.advisories.filter((advisory) => advisory !== pinnedConflictAdvisory)
      ]
    : state.advisories

  if (!visibleAdvisories.length) {
    hideToastAlert()
    return
  }
  ...
  toastAlert.replaceChildren(
    ...visibleAdvisories.map((advisory) =>
      createToastCard({
        ...
        title: advisory.title,
        body: advisory.body,
        dismissKey: getAdvisoryDismissKey(advisory)
      })
    )
  )
  setToastAlertVisible(true)
  emitAdvisoriesUpdated(visibleAdvisories)
}
...
function buildDisplayedFlights(rawFlights) {
  return rawFlights.map((flight) => {
    const squawkOverride =
      state.simulatedPriorityFlight?.icao24 === flight.icao24
        ? String(state.simulatedPriorityFlight.squawk)
        : null
    const sourceFlight = squawkOverride ? { ...flight, squawk: squawkOverride } : flight
    const horizontal = latLonToWorld(sourceFlight.lat, sourceFlight.lon)
    const altitudeMeters = sourceFlight.altitude_m ?? 0
    const world = {
      x: horizontal.x,
      y: altitudeToWorld(altitudeMeters, sourceFlight.on_ground),
      z: horizontal.z
    }
    const currentRefreshPoint = createRefreshFlightPoint(sourceFlight, world)
    const previousRefreshPoint = sourceFlight.is_synthetic
      ? null
      : state.previousRefreshFlightPoints.get(sourceFlight.icao24) ?? null
...
function clearInjectedScenario() {
...
function createFallbackConflictScenarioFlights() {
  ...
  const leadCallsign = createSyntheticCallsign()
  const warnCallsign = createSyntheticCallsign()

  return [
    createSyntheticFlight({
      icao24: createSyntheticFlightId(leadCallsign, "simbase1"),
    ...
    createSyntheticFlight({
      icao24: createSyntheticFlightId(warnCallsign, "simwarn1"),
      callsign: warnCallsign,
      world: eastStart,
      altitudeM: 1219.2,
      headingDegrees: 270,
      speedKnots: 220,
...
function createConflictScenarioFlights() {
  const targetFlight = pickRandomItem(
    state.flights.filter((flight) => !flight.on_ground && !flight.is_synthetic)
  )
  ...
  return {
    ...
    note: `Injected ${warnCallsign} on a reciprocal heading toward ${getFlightDisplayName(targetFlight)} to trigger a conflict advisory.`
  }
}

function createFallbackEmergencyScenario(squawkCode) {
  const dtwAirport = state.airports.find((airport) => airport.code === "KDTW") || state.airports[0]
  const airportWorld = latLonToWorld(dtwAirport.lat, dtwAirport.lon)
  const headingDegrees = pickRandomItem([25, 70, 130, 205, 290, 335]) ?? 70
  const emergencyLabel = EMERGENCY_SQUAWK_LABELS[squawkCode]
  const emergencyWorld = createOffsetWorldPoint(airportWorld, headingDegrees, 5.5)
  const callsign = createSyntheticCallsign()

  return {
    flights: [
      createSyntheticFlight({
        icao24: createSyntheticFlightId(callsign, `sim${squawkCode}`),
        callsign,
        world: emergencyWorld,
        altitudeM: 1676.4,
        headingDegrees,
        speedKnots: 210,
        squawk: String(squawkCode)
      })
    ],
    note: `Injected a simulated ${emergencyLabel.toLowerCase()} squawk ${squawkCode} aircraft near ${dtwAirport.code}.`
  }
}

function createEmergencySquawkScenario(squawkCode) {
  const emergencyLabel = EMERGENCY_SQUAWK_LABELS[squawkCode]
  const targetFlight = pickRandomItem(
    state.flights.filter((flight) => !flight.on_ground && !flight.is_synthetic)
  )
  ...
  if (!targetFlight) {
    return createFallbackEmergencyScenario(squawkCode)
  }

  const airportCode = getNearestAirportContext(targetFlight)?.airport.code || state.region.name
  return {
    flights: [],
    simulatedPriorityFlight: {
      icao24: targetFlight.icao24,
      squawk: String(squawkCode)
    },
    note: `Simulated ${emergencyLabel.toLowerCase()} squawk ${squawkCode} for ${getFlightDisplayName(targetFlight)} near ${airportCode}.`
  }
}

function createRandomConflictScenario() {
  const scenarioChoice = pickRandomItem(["traffic", ...EMERGENCY_SQUAWK_CODES])

  if (scenarioChoice === "traffic") {
    return createConflictScenarioFlights()
  }

  return createEmergencySquawkScenario(scenarioChoice)
}

function injectConflictScenario() {
  clearInjectedScenario()
  const scenario = createRandomConflictScenario()
  state.syntheticFlightsRaw = scenario.flights
  state.simulatedPriorityFlight = scenario.simulatedPriorityFlight ?? null
  state.highlightedConflictIcaos = new Set(
    scenario.flights
...
function drawAirport(airport, basis) {
  ...
  drawWorldLine(anchor, { x: world.x, y: 10, z: world.z }, "#9eb7c6", 1.2, 0.48, [], basis)
...
function runwaySurfaceStyle(runway) {
  ...
  if (runway.surface === "CONC") {
    return {
      fill: "rgba(146, 124, 48, 0.94)",
      edge: "rgba(255, 229, 129, 0.98)",
      glow: "rgba(255, 209, 96, 0.46)"
    }
  }

  return {
    fill: "rgba(122, 101, 34, 0.94)",
    edge: "rgba(255, 220, 112, 0.96)",
    glow: "rgba(255, 195, 86, 0.4)"
  }
}
...
function getProjectedFlightMarker(flight, basis) {
  ...
  const projected = projectPoint(cameraPoint)
  const zoomBoost = getAircraftZoomScaleBoost()
  return {
    projected,
    radius: clamp(projected.scale * 16 * zoomBoost, 10, 34)
  }
}

function getAircraftZoomScaleBoost() {
  const zoomProgress =
    1 - (camera.distance - CAMERA_MIN_DISTANCE) / (CAMERA_MAX_DISTANCE - CAMERA_MIN_DISTANCE)

  return 1 + clamp(zoomProgress, 0, 1) * AIRCRAFT_ZOOM_SIZE_BOOST
}
...
function getFlightColorTheme(flight) {
  if (hasEmergencySquawk(flight)) {
    return FLIGHT_COLOR_THEMES.priority
  }
...
function drawEmergencySquawkLabel(flight, projected, radius) {
  const squawk = getEmergencySquawkCode(flight)
  if (!squawk) {
    return
  }

  const fontSize = clamp(Math.round(radius * 0.78), 13, 17)
  ...
  ctx.lineWidth = 4
  ctx.strokeStyle = "rgba(12, 6, 8, 0.88)"
  ctx.fillStyle = "#ff4f57"
  ctx.strokeText(String(squawk), projected.x, projected.y - radius - 10)
  ctx.fillText(String(squawk), projected.x, projected.y - radius - 10)
  ctx.restore()
}
...
function buildAircraftFaces(flight, basis) {
  ...
  const modelScale = (flight.on_ground ? 1.15 : 1.35) * getAircraftZoomScaleBoost()
...
function drawFlight(flight, basis, showLabel) {
  ...
  const isEmergencyFlight = hasEmergencySquawk(flight)
  const isAlertFlight = isConflictFlight || isEmergencyFlight
  ...
  drawWorldLine(
    ...
    colorTheme.line,
    1.2,
    isAlertFlight ? 0.9 : flight.on_ground ? 0.55 : 0.7,
    isAlertFlight ? [4, 3] : flight.on_ground ? [4, 5] : [7, 6],
    basis
  )
  ...
  if (isEmergencyFlight) {
    drawEmergencySquawkLabel(flight, projected, marker.radius)
  }

  if (showLabel) {
    ctx.fillStyle = isAlertFlight ? "#ffd7d7" : "#eff7fb"
...
function applyFeedSnapshot(payload) {
  if (payload.airspace?.id) {
    state.settings.selectedAirspaceId = getAirspacePresetById(payload.airspace.id).id
  }
...
async function fetchTraffic(force = false) {
  if (disposed || (state.loading && !force)) {
    return
  }

  const requestToken = state.fetchRequestToken + 1
  state.fetchRequestToken = requestToken
  state.loading = true
  ...
  try {
    const searchParams = new URLSearchParams({
      ts: String(Date.now()),
      airspace: state.settings.selectedAirspaceId
    })
    ...
    if (disposed || requestToken !== state.fetchRequestToken) {
      return
    }
...
function bindEvents() {
  ...
  addManagedListener(airspaceSelect, "change", () => {
    const nextAirspaceId = getAirspacePresetById(airspaceSelect.value).id
    if (nextAirspaceId === state.settings.selectedAirspaceId) {
      return
    }

    state.settings.selectedAirspaceId = nextAirspaceId
    saveSettings()
    updateSettingsControls()
    resetDisplayedFeedForAirspace()
    fetchTraffic(true)
  })
  ...
  addManagedListener(toastAlert, "click", (event) => {
    const toastCard = event.target instanceof HTMLElement ? event.target.closest(".toast-card") : null
    if (!toastCard) {
      return
    }

    dismissToastCard(toastCard)
  })
