...
export function mountFlightViewer(elements) {
  ...
  advisoryBanner = elements.advisoryBanner
  advisoryList = elements.advisoryList
...
function rotateCamera(pixelDx, pixelDy) {
  ...
  camera.pitch = clamp(camera.pitch + pixelDy * 0.006, CAMERA_MIN_PITCH, CAMERA_MAX_PITCH)
}
...
function renderAdvisories() {
  ...
  const topAdvisory =
    ...
    createBannerAdvisory(
      "info",
      "Monitoring",
      "No immediate controller advisories",
      "AI-assist heuristics are monitoring for traffic conflicts, sequencing pressure, missed approaches, and priority handling cues."
    )

  advisoryBanner.className = `advisory-banner severity-${topAdvisory.severity}`
  advisoryBanner.replaceChildren()

  const bannerPill = document.createElement("span")
  const bannerTitle = document.createElement("strong")
  const bannerBody = document.createElement("p")
  bannerPill.className = "advisory-pill"
  bannerPill.textContent = topAdvisory.label
  bannerTitle.className = "advisory-title"
  bannerTitle.textContent = topAdvisory.title
  bannerBody.className = "advisory-body"
  bannerBody.textContent = topAdvisory.body
  advisoryBanner.appendChild(bannerPill)
  advisoryBanner.appendChild(bannerTitle)
  advisoryBanner.appendChild(bannerBody)

  advisoryList.innerHTML = ""
  const remainingAdvisories = pinnedConflictAdvisory
    ? state.advisories.filter((advisory) => advisory !== pinnedConflictAdvisory).slice(0, 4)
    : state.advisories.slice(1)

  remainingAdvisories.forEach((advisory) => {
    ...
    item.className = `severity-${advisory.severity}`
    topRow.className = "advisory-card-top"
    pill.className = "advisory-pill"
    pill.textContent = advisory.label
    title.className = "advisory-card-title"
    title.textContent = advisory.title
    body.className = "advisory-body"
    body.textContent = advisory.body
...
function injectConflictScenario() {
  ...
  const conflictAdvisory = getPinnedConflictAdvisory() || state.advisories[0] || null
  showToastAlert({
    severity: conflictAdvisory?.severity || "critical",
    label: conflictAdvisory?.label || "Conflict",
    title: conflictAdvisory?.title || "Injected conflict scenario",
    body: conflictAdvisory?.body || scenario.note
  })
...
function drawTrail(flight, basis) {
  ...
  const trailColor = isConflictHighlightedFlight(flight)
    ? "rgba(255, 92, 92, 0.9)"
    : "rgba(85, 214, 255, 0.82)"
...
function drawFlight(flight, basis, showLabel) {
  ...
  drawWorldLine(
    ...
    flight.world,
    isConflictFlight ? "#ff5c5c" : flight.on_ground ? "#ffcf72" : "#55d6ff",
    1.2,
