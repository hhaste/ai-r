...
function normalizeFlights(rawStates, region) {
  const flights = []
...
export async function buildFeedPayload(airspaceId = DEFAULT_AIRSPACE_ID) {
  const preset = getAirspacePresetById(airspaceId)
  const airportsForAirspace = cloneAirspaceAirports(preset.airports)
  const region = computeAirspaceRegion(preset)
  const runways = filterRunwaysForAirspace(airportsForAirspace)
  const [{ payload: openskyPayload, headers }, metarSnapshot, terrainSnapshot] = await Promise.all([
    OPENSKY.fetchStates(region),
    METAR.fetchLatest(airportsForAirspace),
    TERRAIN.fetchRegion(region)
  ])
  const flights = await enrichFlightsWithRegistry(normalizeFlights(openskyPayload.states, region))
  const sourceLabel = OPENSKY.mode === "oauth" ? "OpenSky OAuth" : "OpenSky Anonymous"
  const airports = attachAirportMetars(airportsForAirspace, metarSnapshot.metars)
  ...
  return {
    airspace: {
      ...
      live_atc_code: preset.liveAtcCode
    },
    region,
    airports,
    runways,
...
function filterRunwaysForAirspace(airports) {
  const airportCodes = new Set(airports.map((airport) => airport.code))
  return RUNWAY_DATA.runways.filter((runway) => airportCodes.has(runway.airport))
}
